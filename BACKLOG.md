...
- **Energy card lifetime-Wh readout on desktop** — `Database::get_capability_lifetime_wh` is wired into MQTT HA discovery since v0.22.0 but the desktop `DeviceEnergy` card only shows 24h/7d/30d windows. Add a "Lifetime" total line (or a "Lifetime" entry in the window toggle) that calls the same function so the desktop surface matches what HA's Energy dashboard integrates. Smaller scoped polish, no migration needed. Deferred from post-v0.22.0 grep survey in favour of Rules observability, which closed a bigger silent-operation gap.
- **Web UI `:9090` Rules toggle + delete parity** — post-v0.22.0 Rules observability shipped Run now + Last triggered on the web UI Rules card, but the pre-existing toggle/delete gap was left alone to keep scope tight. Web admins still have to use the desktop app to enable/disable or delete a rule. Small follow-up: add PATCH `/api/rules/:id/enabled` + DELETE parity buttons to the web card.
