...
- **ESP32 LoadProhibited crash after `Trellis::begin` WiFi timeout** — 2026-04-21 session: serial read on `/dev/ttyUSB0` showed an ESP32 running unedited `BasicSwitch.ino` (SSID = literal "YourWiFi") loop into `WiFi connection timeout` → `Guru Meditation Error: Core 1 panic'ed (LoadProhibited). EXCVADDR: 0x00000008. Backtrace: 0x400d522e,0x400d2edd,0x400d2be4,0x400e6b0c,0x4008e0b5`. Null-pointer deref at offset 8 when `begin()` returns false and the sketch then calls `trellis.loop()`. `Trellis::loop` has a `if (!_webServer) return;` guard, so the deref is somewhere else in the early-return path — possibly `_discovery` or `_provisioning` left dangling. Reproducible on any device whose sketch calls `trellis.begin(wrong_ssid, wrong_pwd)` and then `trellis.loop()` — i.e. the default tutorial flow on a mis-configured device. Low-priority since real devices have correct credentials, but a production-quality library shouldn't crash the device when WiFi fails; at minimum `begin()` should null-guard every pointer it opts not to construct. Needs addr2line on the local build's ELF to map the PC back to source before a fix.

- **Trellis desktop logs are not reachable from shell** — 2026-04-21 session: looked for app logs to verify `log::info!("[OTA] ack: applied row …")` fired after the synthetic ack POST. Neither `~/.local/share/com.trellis.app/logs/`, `~/.cache/trellis/`, nor `journalctl --user -u trellis` found anything. Trellis is launched from the tray/menu, so its stderr goes to `/dev/null` by default. Consider wiring `env_logger` / `tracing` to a rotating file in the app data dir, gated by a `TRELLIS_LOG` env var. Low priority — would have made waiver-proof more rigorous today but the HTTP response bodies + DB stamps already cover the essential signal.
