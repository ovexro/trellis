...
- **OTA delivery to ESP32 failing every attempt — investigate routing/network** — three consecutive `/api/github/ota` triggers against `trellis-fccfb7c8` (192.168.1.108, on the relay subnet) all recorded `delivery_status="failed"` within ~2 seconds of trigger; ESP32 uptime never reset (no flash). The download from Trellis-to-Python-server worked fine; the failure is on the second hop (Trellis-to-device). Likely cause: `ota::serve_firmware` binds to the Trellis machine's local IP (192.168.2.134, picked via UDP-connect to 8.8.8.8), which the ESP32 on 192.168.1.x can't reach despite the OpenWrt Pi5 + socat relay + proxy_arp setup per `project_router_setup.md`. Worth a focused session — either fix the routing or have `serve_firmware` bind to a relay-reachable IP. Surfaced 2026-04-20 while verifying the ota_success_rate rule.
- **mark_firmware_delivery race for concurrent OTAs to the same device** — `db.mark_firmware_delivery` updates "the most recent firmware_history row for the device that has no delivery_status yet". If two OTAs are triggered against the same device in flight simultaneously, outcome A could land on row B and vice-versa. Not currently user-reachable (the UI doesn't expose parallel OTA), and the diagnostics rule reads ratios so per-row mis-attribution doesn't change the verdict. Fix: pass the firmware_history row ID into `serve_firmware` so the update is exact. Defer until concurrent OTAs are a real workflow.
