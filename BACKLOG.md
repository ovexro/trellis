...
- **Device Diagnostics v5: remaining rules** — C3 (power-supply instability via `esp_reset_reason` → brownout clusters) landed post-v0.16.0. Still open: **C2 — flash wear via NVS write frequency** (library-side needs a cheap NVS-write counter, exposed via a new telemetry field; desktop rule reads trend across 24h; requires a schema change), and **C1 — mDNS resolution latency via timed-resolve wrapper in `discovery.rs`** (no library change, pure desktop-side rule reading a new metric). Both are a natural pair for the next release since C3 established the pattern.
