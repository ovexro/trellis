...
- **`:9090` webhook deliveries log surface** — natural follow-up to v0.31.0 slot 1's body capture. The embedded web UI shows webhook cards with last-delivery + success/failure pills but doesn't render the deliveries log itself; only the desktop Automation page does. Promoting `:9090` to render the same expandable per-row log (with the new request/response previews surfaced) would close the parallel-surface gap. The REST endpoint `GET /api/webhooks/:id/deliveries` already returns the new fields so it's pure rendering work.
