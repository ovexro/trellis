...
- **Device Diagnostics v5: C2 — flash wear via NVS write frequency** — C1 (mDNS resolution latency) and C3 (power-supply instability via `esp_reset_reason`) both shipped in v0.17.0. C2 remains: library-side needs a cheap NVS-write counter, exposed via a new telemetry field; desktop rule reads trend across 24h; requires a schema change. Harder to verify without actually wearing flash, so most valuable for multi-month fleets — defer until there's a clearer user signal or we're releasing a batch of library-side changes that can carry it.
- **mdns_latency: broaden capture model (inter-Resolved cadence proxy)** — investigation 2026-04-21 closed: the v0.17.0 rule's capture path is correct, but samples only accrue at new-announcement time (Found → first Resolved). TTL refreshes fire Resolved without Found and are silently skipped, and cross-subnet devices emit no mDNS at all (multicast doesn't cross the router). A healthier model is to record the **inter-Resolved cadence per fullname** — the interval between consecutive Resolved events is itself a LAN-health signal that scales with normal operation. Scope: add a second table or repurpose `device_mdns_latency` with a new sample kind, teach the rule to evaluate cadence drift from baseline, update the UI text accordingly. Medium. See `~/.claude/projects/-home-ovidiu-trellis/memory/project_mdns_latency_capture.md` for the full finding + a 1-minute positive-capture test recipe.
- **Reset events as chart annotations** — `device_reset_history` rows carry a timestamp + reason; a natural follow-up to the v0.17.0 power-supply rule is to surface each reset as a colored vertical marker on the per-device metric charts (reusing the existing annotation rendering). Would require adding a branch in `db::get_annotations` that unions reset rows alongside firmware + state/error/warn logs, and a new `kind: "reset"` in the frontend legend. Deferred — the rule already catches the pattern, this is a visualization polish.
