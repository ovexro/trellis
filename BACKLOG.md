...
- **Energy tracking — MQTT / HA total_increasing Wh sensor** — the power-sensor half shipped (see git log). Follow-up is the cumulative `sensor` entity (device_class=`energy`, state_class=`total_increasing`, unit=Wh) so HA's Energy dashboard picks up Trellis switches. Gotcha (already surveyed): `device_logs severity='state'` is under `data_retention` cleanup, so "lifetime cumulative" drifts past the retention horizon. Acceptable mitigation is to compute against `MIN(timestamp)` in `capability_state_log` and accept the pre-retention drift as a minor artifact HA smooths. Ship when a user asks.
