...
- **OTA "Cancel" / "Abort" button during Downloading** — when an OTA is stuck at 0% (typically the rtl8xxxu router-side flake leaving the TCP send buffer frozen), the user's only recourse is to quit Trellis from the tray to tear down the socket. Neither the desktop OTA page nor the per-device detail panel exposes an abort button. Fix: store the `stop_flag: Arc<Mutex<bool>>` that `ota::serve_firmware` already returns, pair it with the `device_id`, and render a "Cancel" button on the OTA progress row that sets the flag. serve_firmware checks the flag in its read loop and shuts down the listener; device will see connection reset and HTTPUpdate will surface an error to the user. Caught in the 2026-04-20 v0.15.0 hardware-test session.
