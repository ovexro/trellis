...
- **Energy tracking — sliders phase 2** — v0.19.0 only meters bool-valued `update` events (switches). Sliders carry PWM duty that has a non-linear relationship to power draw. Two possible designs: (a) assume linear — `nameplate_watts × (value / max)` — fine for heater elements and resistive loads, wrong for LED drivers with CC/CV curves; (b) let user enter a 0-100% → watts lookup. Start with (a) as an opt-in checkbox on the slider's CapabilityWatts field; add (b) only if a user asks.
- **Energy tracking — online-aware on-time** — current derivation keeps accruing on-time if the device goes offline with a switch left ON, because we can't see any further state events. Correct thing to do: intersect capability on-time with the `device_logs severity='state'` online intervals so offline periods don't inflate Wh. Pure backend change in `get_device_energy`.
- **Energy tracking — web dashboard parity on :9090** — desktop-only in v0.19.0. Port `CapabilityWatts` inline editor + `DeviceEnergy` section into `web_ui.html` so tunnel users see the same numbers.
- **Energy tracking — MQTT / HA discovery** — once the Wh stream is stable, mirror each metered switch as an HA `sensor` entity (device_class: `energy`, state_class: `total_increasing`) so energy history shows up in HA's Energy dashboard. Read from existing `get_device_energy` cumulatively.
...
- **Energy tracking — "energy_coverage" diagnostics rule** — suggested in the approved MVP scope but deferred. INFO until at least one switch on the device has `nameplate_watts` set; OK once configured. Pure visibility nudge.
