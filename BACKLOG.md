...
- **Device Diagnostics v2: fleet drill-in** — the Home fleet-health widget shows counts + device list; next step is to surface the top-offending finding per device inline (e.g. "heap leak · free heap down 1.2KB/h") without requiring a click-through to the device detail page.
