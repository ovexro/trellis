...
- **Energy tracking — sliders phase 3 (non-linear lookup)** — phase 2 shipped in v0.22.0 as the linear opt-in (design (a) in the original sketch: `nameplate_watts × (value / max)` gated by a `linear_power` checkbox + `slider_max` column on `capability_meta`). Phase 3 would layer a 0-100% → watts lookup table for LED drivers with CC/CV curves, motor torque curves, etc. Only worth shipping when a user files a concrete case where linear is visibly wrong — heater/incandescent loads cover most practical setups today.
- **Energy tracking — hardware meters (INA219/PZEM)** — long tail. Library-side addition: new capability type `energy_meter` with live W / Wh reporting. Desktop side reuses the existing `DeviceEnergy` renderer but sources from metric rows instead of derived on-time.
- **Multi-line scrollable text capability** (community ask, [#2](https://github.com/ovexro/trellis/issues/2)) — current `text` capability is single-line; extend with a multi-line + `overflow:auto` render mode. Incremental change to the existing capability rather than a new one. v0.33.0 slate candidate.
- **Gauge render mode for `sensor` capability** (community ask, [#2](https://github.com/ovexro/trellis/issues/2)) — radial or bar gauge with min/max bounds, ideal for temperature / speed / pressure. Stacks on top of the existing numeric `sensor` stream; no protocol change. v0.33.0 slate candidate.
- **Momentary switch capability** (community ask, [#2](https://github.com/ovexro/trellis/issues/2)) — on while pressed, off on release. Distinct from latching `switch`. Either a new capability or a `mode: "momentary"` flag on `switch` (decide at design time). v0.33.0 slate candidate.
- **Peer-aware scenes — scenes that touch caps on other LAN peers** — v0.33.0 slot 2 ships scenes scoped to a single device's own caps. A natural extension is "Living Room On" → switches every relay on every device tagged Living Room across the LAN. Needs a cross-device WS auth story (the embedded WS is unauthenticated today; opening it to remote scene-recall by other devices is a different threat model than slot 1's read-only `/api/info` browse). Defer until slot 1 + 2 see real-user feedback.
- **Timezone / DST for on-device schedules** — v0.33.0 slot 2 schedules fire on wall-clock UTC. Adding a `Trellis::setTimezoneOffset(int minutes)` library API + a per-device offset setting persisted in NVS, threaded through `gmtime_r` → local-time, would let users pick a real local-time fire moment. DST handling needs either a tz-database lookup (heavy) or a per-cut "IANA shorthand → fixed offset" table (lighter but lossy for half the year). v0.34.0 polish candidate.
- **Pico (RP2040) scenes parity** — Pico builds the v0.33.0 slot 2 surface but `TrellisScenes` is no-op on RP2040 (no NVS, no SNTP `configTime`, no `gmtime_r` in the same shape). Could add LittleFS-backed persistence (already used for WiFi creds fallback in `TrellisProvisioning`) + an embedded-time source for the tick. Unblocks Pico W users who want scenes/schedules. Lower priority — slot 1 set the same Pico-no-op precedent and the user base seems heavily ESP32-tilted.
- **Windows desktop binary** (community ask, [#1](https://github.com/ovexro/trellis/issues/1)) — Tauri supports Windows builds; would need a Windows runner in CI plus the cross-platform release flow extended. Unblocks Windows-only users who can't or won't use WSL2. Larger than a single slot — likely its own session.
- **ARM64 desktop binary (Pi 4/5 / Apple Silicon Linux)** (community ask, [#1](https://github.com/ovexro/trellis/issues/1)) — current `.deb`/`.rpm`/`.AppImage` are amd64 only, so users with HA on a Pi can't install the desktop app on the same box. Would need an `aarch64` runner in release CI. Pairs naturally with the Windows-build effort.
