...
import { Download, Upload, Check, Bell, Radio, Key, Copy, Trash2, AlertTriangle, Globe, ExternalLink } from "lucide-react";
...
export default function Settings() {
  ...
  // Remote access reachability probe state. The widget asks the user to
  // paste a token and a public URL, then runs `probe_remote_url` against
  // `<url>/api/devices` to verify the tunnel + token combo works
  // end-to-end. The token is held only in component state and never
  // persisted — the user retypes it for each probe (no risk of stashing
  // plaintext anywhere). The persisted public URL is just a convenience
  // so the user doesn't have to retype the hostname between probes.
  ...
  const runReachabilityProbe = async () => {
    ...
    try {
      ...
      // Persist the URL (NOT the token) so the next probe doesn't make
      // the user retype the hostname. The token never leaves component
      // state — it would be a footgun to stash plaintext anywhere.
      try {
      ...
      <div className="space-y-8">
        ...
        <div>
          ...
          <div className="space-y-3">
            ...
            {/* Safety check: warn loudly if zero tokens minted, because in
                that case the loopback bypass is the *only* path through the
                gate, and a remote tunnel arrives from a non-loopback peer
                that has no way to authenticate. The user MUST mint a token
                first or the tunnel will be unreachable. */}
            {apiTokens.length === 0 && (
