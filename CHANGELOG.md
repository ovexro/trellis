...
- **Sketch generator core.** New `sketch_gen` module turns a `SketchSpec { device_name, firmware_version?, capabilities: [{kind, id, label, ...}] }` into a complete Arduino `.ino` source. Supported capability kinds: `switch`, `slider`, `color`, `sensor`. Output covers `#include <Trellis.h>`, WiFi placeholders, the `Trellis trellis(...)` constructor, a stubbed `onCommand` handler with per-slider/color branches, and a `setup()` body emitting one `addX` call per capability. Strict validation (identifier regex, label/device-name no-newline/no-quote, GPIO 0-48, slider min<max, ≤16 caps, unique cap IDs, semver firmware version) returns descriptive `400` errors. New Tauri command `generate_sketch_command` + admin-gated REST `POST /api/sketch/generate` returning `{"sketch": "..."}`. Hardware-verified by compiling a representative all-kinds output via `arduino-cli` against the published 0.27.0 library on both `esp32:esp32:esp32` (90% flash, 16% RAM) and `rp2040:rp2040:rpipicow` (22% flash, 28% RAM). Wizard UI deferred to a subsequent slot — this lands the engine + endpoint only. 17 new lib tests (214 → 231).
